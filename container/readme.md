...
This container is one component of the [qemu-pfsense](../README.md) project: it does **not** manage bridges, TAPs, or firewalld on the host side. Its only job is to cleanly boot the pfSense VM with the network interfaces it is handed, and to correctly report its health status to the rest of the stack.
...
These values can be overridden in the quadlet file (`services/etc/containers/systemd/qemu-pfsense.container`), not in this Dockerfile. Other variables (`KVM`, `ARGUMENTS`, `VERSION`, `TIMEOUT`, …) remain those of the upstream `qemux/qemu` image — refer to its own documentation for their general usage.
...
It's this (repeated) container failure that then propagates to the host systemd level (`StartLimitBurst`, `OnFailure=qemu-pfsense-recovery.service`) and triggers, if needed, the firewalld fallback described in the [main README](../README.md#recovery).
...
| Item            | Value                                                        | Role                                        |
|-----------------|--------------------------------------------------------------|---------------------------------------------|
| `Volume`        | `/run/qemu-pfsense/network.env:/tmp/qemu-tap-pfsense.env:ro` | List of TAPs to inject (generated by the    |
|                 |                                                              | host)                                       |
...
podman build -t qemu-pfsense:current ./container
...
podman run --rm -it \
  ...
  -v /run/qemu-pfsense/network.env:/tmp/qemu-tap-pfsense.env:ro \
  -v ./storage:/storage \
  localhost/qemu-pfsense:current
```

> In normal use, this container is **never started manually**: it is launched by the `qemu-pfsense.container` quadlet, which itself depends on `qemu-pfsense-bridges.service` (see the [main README](../README.md)).

## 7. Troubleshooting

```bash
# Podman healthcheck status
podman inspect --format '{{.State.Health.Status}}' qemu-pfsense

# Container logs (start.sh, watchdog, qemu)
journalctl -u qemu-pfsense.service -f

# Manual QMP dialogue from inside the container
podman exec -it qemu-pfsense sh -c \
